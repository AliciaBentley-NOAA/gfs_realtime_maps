...
echo "Most recent run: ${start_date}${start_hh}Z"
echo "Cycle order: ${all_cycles[@]}"
...
# Calculate target time in seconds
    ...
    for plot_type in "${plot_types[@]}"; do
        local_path="/lfs/h2/emc/ptmp/alicia.bentley/gfs_realtime_maps/maps/gfs/${YYYYMMDD}${HH}/${plot_type}"
        remote_path="/home/people/emc/www/htdocs/users/meg/gfsv17/realtime/images/conus/${plot_type}/p${FFF}"
        
        if [ -d "$local_path" ]; then
            echo "Copying to emcrzdm: ${YYYYMMDD}${HH} | Offset: p${FFF} | Type: ${plot_type}"
